...
module virtualNetwork 'modules/vwan/vnet.bicep' = if (deployVnetConnection == true) {
...
module vnetConnection 'modules/vwan/vnetconnection.bicep' = if (deployVnetConnection) {
  scope: vwanRG
  name: 'deploy-vnetconnection'
  params: {
    //hubName: vwanHubName
...
module vpnConnection 'modules/vwan/vwanhubvpnconnection.bicep' = if (deployS2SConnection) {
  scope: vwanRG
...
output avsRGName string = privateCloudRG.name
